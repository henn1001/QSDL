...
class TestSpringSpecifics:
    ...
    def test_string_ids_use_external_uid_in_services(self) -> None:
        ...
        files = SpringTestUtils.generate(schema, config={"id_type": "STRING"})

        service_content = SpringTestUtils.read_file(files, "src/main/java/app/server/service/ProjectService.java")
    ...
    def test_no_database_omits_persistence_artifacts(self) -> None:
        ...
        files = SpringTestUtils.generate(schema, config={"database": "NO"})

        configuration_content = SpringTestUtils.read_file(
            files, "src/main/java/app/server/config/AppConfiguration.java"
        )
    ...
    def test_package_placeholders_use_directives_and_namespace(self) -> None:
        ...
        for relative_path in (
            ...
            assert SpringTestUtils.file_exists(files, relative_path)

        ticket_content = SpringTestUtils.read_file(files, "src/main/java/app/server/incident/dto/Ticket.java")
