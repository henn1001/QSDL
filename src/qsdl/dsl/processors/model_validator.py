...
def validate_field_directives(schema: dsl.Schema, metamodel: textx.metamodel.TextXMetaModel) -> None:
    ...
    for entity in bases + objects:
        ...
        for field in entity.fields:
            ...
            # verify that opaque is used only for Bases on non-arrays and do not contain relations
            if field.is_opaque and field.value._tx_fqn != "entity.Base":
                ...
                raise TextXSemanticError(msg, filename=schema._tx_filename)
            if field.is_opaque and field.is_array:
                msg = f"The Field {field.name} for {field.parent.name} declares opaque for an array value."
                raise TextXSemanticError(msg, filename=schema._tx_filename)
            if field.is_opaque and [
                field for field in field.value.fields if field.is_aggregation or field.is_composition
            ]:
                msg = f"The Field {field.name} for {field.parent.name} declares opaque for a relation value."
